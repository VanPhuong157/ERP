  ...
  const renderProgressRow = (emp: any) => {
    ...
    return (
      <tr key={emp.id} className="border-b hover:bg-slate-50 transition-colors">
        ...
        <td className="p-4 text-center text-emerald-600 font-bold">{completed}</td>
        <td className="p-4 text-center">
          {pending > 0 ? (
            ...
            <span className="text-slate-400 font-bold text-[10px] uppercase italic">Đã hoàn thành</span>
          )}
        </td>
        <td className="p-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] font-bold text-slate-500 w-8 text-right">{percent.toFixed(0)}%</span>
          </div>
        </td>
      </tr>
    );
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Tổng việc</p>
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm border-l-4 border-l-emerald-500">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Hoàn thành</p>
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm border-l-4 border-l-blue-500">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Cố gắng</p>
              <p className="text-3xl font-black text-blue-600 mt-1">{data?.sectionKPI?.completedEffort || 0}</p>
            </div>
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm border-l-4 border-l-orange-500">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Cảnh báo</p>
              <p className="text-3xl font-black text-orange-500 mt-1">{data?.sectionKPI?.completedWarning || 0}</p>
            </div>
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm border-l-4 border-l-red-500">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Không đạt</p>
              <p className="text-3xl font-black text-red-500 mt-1">{data?.sectionKPI?.notMet || 0}</p>
            </div>
          </div>
              ...
              <table className="w-full text-left border-collapse text-sm">
                ...
                <tbody>
                  ...
                  <tr className="border-b bg-blue-50/30 font-bold text-blue-700">
                    ...
                    <td className="p-4 text-center">{data?.sectionKPI?.completedEffort || 0}</td>
                    <td className="p-4 text-center">{calculatePercent(data?.sectionKPI?.completedEffort, totalKPI)}%</td>
                  </tr>
                  ...
                  <tr className="bg-red-50/30 font-bold text-red-700">
                    ...
                    <td className="p-4 text-center">{data?.sectionKPI?.notMet || 0}</td>
                    <td className="p-4 text-center">{calculatePercent(data?.sectionKPI?.notMet, totalKPI)}%</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div className="mt-8 space-y-4">
            <div className="flex items-center gap-2 border-l-4 border-red-600 pl-4">
              ...
              <h2 className="font-bold text-slate-700 uppercase text-sm">
                {targetDeptId ? "Theo dõi tiến độ nhân viên (Việc chưa xong)" : "Tiến độ công việc của tôi"}
              </h2>
            </div>
              ...
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="bg-slate-50 font-bold text-[10px] uppercase text-slate-500 border-b">
                    <th className="p-4">{targetDeptId ? "Nhân viên" : "Hồ sơ"}</th>
                    <th className="p-4 text-center">Tổng việc</th>
                    <th className="p-4 text-center text-emerald-600">Đã xong</th>
                ...
                <tbody>
                  {targetDeptId && !filter.userId ? (
                    data?.employeeList?.map((emp: any) => renderProgressRow(emp))
                  ) : (
                    renderProgressRow({
                      id: filter.userId || user?.id,
                      fullName: employees.find(e => e.id === filter.userId)?.fullName || user?.name || "Nhân viên",
