...
export default function CustomerManagementPage() {
  const [customers, setCustomers] = useState<Customer[]>([]); // Khởi tạo mảng rỗng
  ...
  const openModal = (customer?: Customer) => {
    if (customer) {
      ...
      setFormData({
        id: '', maKhachHang: '', tinh: '', kcnCcn: '', tenKhachHang: '',
  ...
  const handleSave = async () => {
    try {
      ...
      await fetchCustomers(); // Load lại danh sách mới nhất
  ...
  return (
    <div className="space-y-4 p-4 bg-slate-50 min-h-screen font-sans">
      ...
      {/* Table Area - GIỮ NGUYÊN GIAO DIỆN */}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-[11px] text-left border-collapse min-w-[2500px]">
            ...
            <tbody className="divide-y divide-slate-200">
              {loading ? (
                <tr><td colSpan={22} className="text-center py-10 text-slate-400">Đang tải dữ liệu...</td></tr>
              ) : (
                customers.map((c, index) => (
                  <tr key={c.id} className="hover:bg-slate-50 transition-colors text-slate-700 font-medium">
                    ...
                    <td className="px-3 py-3 border text-center">{c.ngayKyHopDong?.split('T')[0]}</td>
                    ...
                    <td className="px-3 py-3 border text-center sticky right-0 bg-white shadow-[-4px_0_10px_rgba(0,0,0,0.05)]">
                      <div className="flex justify-center gap-2">
                        ...
                        <button onClick={() => handleDelete(c.id)} className="p-1.5 text-rose-600 hover:bg-rose-50 rounded-lg"><Trash2 size={14} /></button>
                      </div>
                    </td>
                  </tr>
                ))
      ...
      {/* Form Modal - GIỮ NGUYÊN GIAO DIỆN */}
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-5xl w-full max-h-[90vh] overflow-hidden flex flex-col">
            <div className="p-5 border-b flex justify-between items-center bg-slate-50">
              <h2 className="font-black text-[#1e3a5f] uppercase tracking-wide">{editingId ? 'Cập nhật' : 'Thêm mới'} khách hàng</h2>
            ...
            <div className="p-6 overflow-y-auto grid grid-cols-1 md:grid-cols-4 gap-4">
              {/* Tất cả các trường input giữ nguyên 100% */}
              ...
              <div className="space-y-1">
                ...
                <input type="date" value={formData.ngayKyHopDong?.split('T')[0]} onChange={(e) => setFormData({ ...formData, ngayKyHopDong: e.target.value })} className="w-full px-3 py-2 border rounded-lg" />
              </div>
              <div className="space-y-1">
