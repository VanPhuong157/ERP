...
import {
  Menu, LogOut, Package, Wrench, Calendar, Warehouse, BarChart3,
  ChevronDown, ChevronRight, Building2, Users, Briefcase, Wallet,
  Truck, ShieldCheck, FileSpreadsheet, UserPlus, Award 
} from "lucide-react";
...
interface User {
  ...
  company: string; deptId: string; deptName: string;
}
...
export default function App() {
    ...
    const loadSavedAuth = () => {
      try {
        ...
        if (token) {
          ...
          setUser(userData);
          setActiveDept({ id: userData.deptId, name: userData.deptName, companyId: userData.company });
        }
  ...
  // --- Cấu trúc Menu Đầy Đủ (Đã bỏ Chairman) ---
  const menuStructure = [
    {
      id: "admin-section",
    ...
    },
    {
      ...
      children: [
        { 
          ...
          icon: Award, 
          deptId: "4", 
          accessRoles: ["CHAIRMAN", "BOD", "ADMIN"], // Cả Chairman và BOD đều vào đây
          children: [
    ...
    },
    {
      ...
      children: [
        ...
        { id: "vhs-ketoan", label: "Phòng Tài chính-Kế toán", icon: Wallet, deptId: "6", children: [{ id: "dept-dashboard", label: "Báo cáo Dashboard", icon: BarChart3 }, { id: "task-registration", label: "Đăng ký nhiệm vụ", icon: FileSpreadsheet }] },
        { id: "vhs-hcns", label: "Phòng HCNS", icon: Users, deptId: "7", children: [{ id: "dept-dashboard", label: "Báo cáo Dashboard", icon: BarChart3 }, { id: "task-registration", label: "Đăng ký nhiệm vụ", icon: FileSpreadsheet }] },
        { id: "vhs-kehoach", label: "Phòng Kế hoạch - Đầu tư", icon: Calendar, deptId: "2", children: [{ id: "dept-dashboard", label: "Báo cáo Dashboard", icon: BarChart3 }, { id: "task-registration", label: "Đăng ký nhiệm vụ", icon: FileSpreadsheet }] },
        { id: "vhs-kythuat", label: "Phòng Kỹ thuật-Công nghệ", icon: Wrench, deptId: "8", children: [{ id: "dept-dashboard", label: "Báo cáo Dashboard", icon: BarChart3 }, { id: "task-registration", label: "Đăng ký nhiệm vụ", icon: FileSpreadsheet }] },
        { id: "vhs-duan", label: "Phòng Quản lý dự án", icon: Briefcase, deptId: "9", children: [{ id: "dept-dashboard", label: "Báo cáo Dashboard", icon: BarChart3 }, { id: "task-registration", label: "Đăng ký nhiệm vụ", icon: FileSpreadsheet }] },
        { id: "vhs-kinhdoanh", label: "Phòng Kinh doanh", icon: Users, deptId: "10", children: [{ id: "dept-dashboard", label: "Báo cáo Dashboard", icon: BarChart3 }, { id: "task-registration", label: "Đăng ký nhiệm vụ", icon: FileSpreadsheet }] },
        { id: "vhs-logistics", label: "Phòng Logistics", icon: Truck, deptId: "11", children: [{ id: "dept-dashboard", label: "Báo cáo Dashboard", icon: BarChart3 }, { id: "task-registration", label: "Đăng ký nhiệm vụ", icon: FileSpreadsheet }] },
      ]
    }
  ];
  ...
  const getFilteredMenu = (items: any[], parentDeptId: string | null = null, parentId: string | null = null): any[] => {
    ...
    return items
      ...
      .map(item => {
        ...
        if (item.children) return { ...item, deptId: currentDeptId, parentCompanyId: currentParentId, children: getFilteredMenu(item.children, currentDeptId, currentParentId) };
      ...
      <div key={item.id} className="w-full">
        <button
          onClick={() => {
            if (hasChildren) {
              ...
              if (item.deptId) {
                setActiveDept({ id: item.deptId, name: item.label || "Phòng ban", companyId: item.parentCompanyId });
              }
            }
          }}
        ...
        {hasChildren && isOpen && sidebarOpen && (
          <div className="bg-slate-950/20">
            {item.children.map((child: any) => <NavItem key={child.id} item={child} level={level + 1} />)}
          </div>
        )}
      </div>
    );
  ...
  const renderPage = () => {
    switch (currentPage) {
      case "dept-dashboard": return <DashboardPage targetDeptId={activeDept?.id} title={`Báo cáo ${activeDept?.name}`} />;
  ...
  if (!user) return <AppContext.Provider value={{ user, setUser, currentPage, setCurrentPage }}><LoginPage /></AppContext.Provider>;
      ...
      <div className="flex h-screen bg-slate-100 font-sans text-slate-900">
        <aside className={`${sidebarOpen ? "w-64" : "w-20"} bg-slate-900 text-white transition-all duration-300 flex flex-col z-30`}>
          ...
          <div className="border-t border-slate-800 p-4 space-y-2">
            ...
            <button onClick={() => setSidebarOpen(!sidebarOpen)} className="w-full flex items-center justify-center p-2 text-slate-600 hover:text-white mt-2">
              <Menu size={20}/>
            </button>
          </div>
        </aside>
